# Section 13 — Negotiation

**Length:** 12 days. **Goal:** get real outcomes when interests compete — a price, a deadline, a disagreement about who does what.

---

### Day 1 — Know Your Walk-Away Point First

Before anything else, know your BATNA — your Best Alternative To a Negotiated Agreement, i.e. what you'll actually do if this falls through. If you're negotiating a salary and your BATNA is "I already have another offer at $X," you know exactly where your floor is. If your BATNA is "I have no other options," you know you're negotiating from a weaker position — and that's useful to know before you start, not mid-conversation.

| Min | Do |
|---|---|
| 15 | Baseline: recall your last real negotiation, however small. How did you approach it? Did you know your BATNA? |
| 20 | For a real upcoming or recent negotiation, write your actual BATNA and walk-away point. |
| 15 | Was your baseline story negotiated blind, or did you actually know your fallback? |
| 10 | Reflection. |

### Day 2 — Where Your Leverage Actually Comes From

Leverage isn't fixed — it comes from your BATNA, how urgent this is for each side, and who has more information. It can shift mid-negotiation as you learn more about their situation.

**Ask yourself:** who needs this deal more, right now? Whoever can walk away more easily has more leverage, regardless of who "should" have the upper hand on paper.

| Min | Do |
|---|---|
| 5 | Recall yesterday's BATNA. |
| 25 | For 2 real or upcoming negotiations, list what gives you leverage and what gives the other side leverage. |
| 20 | Does your leverage change if you wait, or gather more information first? |
| 10 | Reflection. |

### Day 3 — The First Number Matters More Than You Think

Whoever states a number first shapes the whole range that follows, disproportionately — even when the other side knows the anchor is a negotiating tactic, it still pulls the outcome toward it. Know when to anchor first, and how to respond to someone else's anchor without over-conceding to it.

**Responding to their anchor:** don't counter close to their number — that silently validates it as the real range. Counter from your own, separately-reasoned number, then work toward the middle from there.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Open a low-stakes real or roleplayed negotiation with a deliberate, justified anchor. Also practice responding to someone else's anchor without over-conceding. |
| 15 | Did anchoring first feel uncomfortable? What justification made it feel legitimate? |
| 10 | Reflection. |

### Day 4 — Giving Something Up on Purpose

A concession only works if you signal it has real value and ask for something back — quietly giving in just invites more asks, because it teaches the other side that pushing works.

**Format:** "I can do X, if you can do Y" — never just "okay, fine, X" with nothing in return.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Roleplay a negotiation where you concede something small deliberately, signal its value, and ask for something in return. |
| 15 | How does a signaled concession feel different from just giving in? |
| 10 | Reflection. |

### Day 5 — Checkpoint

| Min | Do |
|---|---|
| 5 | Recall this week's BATNA and leverage notes. |
| 30 | Real or detailed roleplay negotiation using BATNA prep, a deliberate anchor, and a signaled concession. |
| 15 | Rep on weakest element. |
| 10 | Progress vs. Day 1. |

### Day 6 — What Do They Actually Want

Ask open questions before you make any offer — what people first state often isn't their real priority, and you can't trade well until you know what actually matters to them. Someone insisting on a specific price might actually care more about timeline, and would happily trade on price if you moved faster.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | In a negotiation roleplay, ask 3 open questions before making any offer, to find out what they actually care about. |
| 15 | Did their stated priority match what came out after questioning? |
| 10 | Reflection. |

### Day 7 — Making Your Case Land

Frame your position around what the other side actually cares about — credibility, real stakes, and sound logic together — not just stating your reasoning and hoping it lands on its own merits. **Worked example.** Logic-only: "This price is fair based on market rates." Combined: "I've done three deals like this in the last year at this rate (credibility), and if we go lower I genuinely can't deliver the quality you're asking for (stakes) — here's the breakdown of where the cost actually goes (logic)."

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Take a real negotiation position. Reframe your pitch around the other side's likely concern. |
| 15 | How is persuading-to-agree different from just stating your position? |
| 10 | Reflection. |

### Day 8 — Finding the Trade Both Sides Actually Want

If one side cares more about timeline and the other more about price, there's a trade that gives both more of what they actually want — but only once you know their real priorities from Day 6. This is the whole point of asking first: without it, you're just splitting the difference on the wrong axis.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Given two parties' stated priorities, find a trade that gives both more of what they care about most. |
| 15 | Was there a non-obvious trade you found only after really understanding both sides? |
| 10 | Reflection. |

### Day 9 — Staying Calm Against a Difficult Counterpart

Match their aggression or evasiveness and you lose the negotiation to your own reaction, not to their tactics. Stay regulated (the pause from Section 5) and hold your position calmly instead — calm, repeated firmness outlasts aggression almost every time.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Roleplay a negotiation against an aggressive or evasive counterpart. Practice calm, non-mirrored responses. |
| 15 | What's your instinct under pressure — match their energy, or shut down? What will you do instead? |
| 10 | Reflection. |

### Day 10 — Checkpoint: Full Negotiation

| Min | Do |
|---|---|
| 5 | Recall the full toolkit. |
| 35 | Full, harder negotiation roleplay combining questions-first, anchoring, concessions, win-win framing, and a difficult moment. |
| 20 | Progress vs. Day 5. |

### Day 11 — Real Negotiation

| Min | Do |
|---|---|
| 10 | Identify one real negotiation opportunity this week — price, deadline, terms, a household ask. |
| 35 | Prepare it (BATNA, priorities) and run it for real. |
| 15 | Report the full process and outcome. |

### Day 12 — Exit Check

| Min | Do |
|---|---|
| 5 | Recall the full toolkit. |
| 40 | A real or professional-context negotiation using: prepared BATNA, 2+ open questions before offering, a deliberate anchor, a signaled concession, and a win-win attempt. Report the full process. |
| 15 | Compare to Day 1's baseline — what's structurally different now? |

**Mastery bar:** you prepare BATNA and ask questions before offering, automatically. If not yet, repeat BATNA-prep and difficult-personality drills on fresh scenarios.
