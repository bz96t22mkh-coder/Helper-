# Section 17 — Financial Literacy

**Length:** 14 days. **Goal:** the general financial literacy that sits underneath your trading study. Educational, not personalized advice — verify anything real-money with a qualified source.

---

### Day 1 — Net Worth and Cash Flow

Net worth is assets minus liabilities — the clearest single snapshot of where you actually stand. Cash flow is money in minus money out over time, and it's a different number: you can have healthy net worth and negative cash flow, or the reverse. An asset puts money in your pocket or holds value; a liability takes money out or is owed.

| Min | Do |
|---|---|
| 10 | Baseline: rate your financial knowledge 1–5 on budgeting, debt/interest, investing. Do you track your net worth or cash flow today? |
| 25 | List your real assets and liabilities. Calculate your actual current net worth. |
| 15 | Begin tracking real income vs. expenses this week. |
| 10 | Did the net worth number surprise you? |

### Day 2 — Building a Budget

Track income vs. expenses, then deliberately split allocations — needs, wants, savings — instead of spending reactively and hoping it works out.

| Min | Do |
|---|---|
| 5 | Recall your net worth number. |
| 30 | Continue tracking real cash flow. Draft a needs/wants/savings budget split from your real numbers. |
| 15 | Where do you suspect money is leaking without a clear budget? |
| 10 | Reflection. |

### Day 3 — Emergency Fund

3–6 months of essential expenses set aside before other financial goals. It's what keeps one bad month from becoming a crisis.

| Min | Do |
|---|---|
| 5 | Recall your cash-flow tracking so far. |
| 25 | Calculate 3–6 months of your real essential expenses as an emergency fund target. Compare to your current savings. |
| 20 | How far are you from that target, and what's one realistic step toward it? |
| 10 | Reflection. |

### Day 4 — Debt, and the Math of Interest

Good debt can build an asset or earning capacity at a reasonable rate. Bad debt is high-interest and funds things that lose value. Either way, unpaid interest compounds against you — the exact same mechanism that grows savings, just running in reverse.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 20 | Classify 3 real or hypothetical debts as generally good or bad, and why. |
| 20 | Manually calculate how a credit-card-style balance grows if unpaid over 12 months at a typical high rate. |
| 15 | What did seeing the actual math change about how you think about debt? |

### Day 5 — Checkpoint

| Min | Do |
|---|---|
| 5 | Recall net worth and emergency fund target. |
| 30 | Finalize this week's cash flow tracking and a first-pass budget. |
| 15 | Rep on weakest element. |
| 10 | Progress vs. Day 1. |

### Day 6 — Compound Interest and Inflation

The same mechanism that grows debt against you grows savings for you, in the other direction. Inflation is why cash sitting idle for years quietly loses real value — which is the actual argument for investing instead of just saving.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Manually work through how a savings sum grows over 10/20/30 years at a given rate — the same mechanism as Day 4's debt calculation, opposite direction. |
| 15 | What does inflation mean for cash sitting idle long-term? |
| 10 | Reflection. |

### Day 7 — Banking and Credit

Your credit score is driven by payment history, how much of your available credit you're using, and how long you've had it. It matters because it affects every future borrowing cost.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | List the main factors that affect a credit score and where you currently stand, if known. |
| 15 | Any concrete action to protect or improve your credit standing? |
| 10 | Reflection. |

### Day 8 — What a Stock, Bond, and ETF Actually Are

A stock is ownership in a company. A bond is a loan to a company or government, paid back with interest. An ETF is a bundled basket of many stocks or bonds, bought as one unit. That's the whole plain-language version.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Explain the difference between a stock, a bond, and an ETF in your own words, no jargon, as if to a beginner. |
| 15 | Where did you have to look something up to get it right? |
| 10 | Reflection. |

### Day 9 — Why Diversification Actually Works

Putting everything into one company means one bad event can wipe you out. Spreading across uncorrelated assets means no single event can — that's the entire logic, not a superstition.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Explain in your own words, with a concrete example, why one company is riskier than a diversified basket. |
| 15 | How does this connect to anything you're already studying in trading? |
| 10 | Reflection. |

### Day 10 — Checkpoint

| Min | Do |
|---|---|
| 5 | Recall this week's concepts. |
| 30 | Explain compound interest, diversification, and one investment vehicle in your own words. Finalize this week's budget. |
| 15 | Rep on weakest concept. |
| 10 | Progress vs. Day 5. |

### Day 11 — One Connected Plan

Budget, emergency fund, and investing aren't separate silos — they're one continuous plan, each step feeding the next.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Draft a real plan: current budget → emergency fund timeline → a general savings/investing goal, connected as one plan. |
| 15 | What's the very next real action this plan calls for? |
| 10 | Reflection. |

### Day 12 — Spotting a Scam

Guaranteed high returns, pressure to act immediately, unverifiable sources, "too good to be true" — these are the actual red flags, especially relevant since trading spaces get targeted heavily.

| Min | Do |
|---|---|
| 5 | Warm-up. |
| 30 | Review 3 real or example financial/trading pitches. Identify the red flags in each. |
| 15 | Has anything you've encountered in trading-adjacent spaces shown these red flags? |
| 10 | Reflection. |

### Day 13 — Your Real Numbers

| Min | Do |
|---|---|
| 5 | Gather your real numbers from the week. |
| 45 | Finalize your complete real snapshot: net worth statement, a month of cash flow, working budget, emergency fund target and timeline. |
| 10 | What's most different about how you see your finances now vs. Day 1? |

### Day 14 — Exit Check

Submit your complete financial snapshot plus a teach-it explanation of compound interest, diversification, and one investment vehicle.

| Min | Do |
|---|---|
| 5 | Recall the full plan. |
| 40 | The full exit task. |
| 15 | Compare to Day 1's self-ratings — where did the biggest shift happen? |

**Mastery bar:** you track net worth and cash flow, and understand debt, interest, and basic investing clearly enough to explain them. If not yet, repeat the weakest concept with fresh calculations. This stays a light ongoing habit — revisit monthly.
