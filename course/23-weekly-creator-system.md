# Module 23 — Your Weekly Creator System
...
FILM → SEND TO EDITOR → REVIEW → UPLOAD → SHORTS →
...
### RESEARCH
Quick trend-relevance check (Module 1/6) — is the topic still hot, or has the window passed? Competitor-gap check if relevant.

### TITLE (Module 7)
Generate 10–20 title variants for the selected idea. Pick your top 1 (with a backup) before scripting — title clarity often sharpens the actual content plan.

### THUMBNAIL CONCEPT (Module 8)
Draft the Thumbnail Brief template — focal point, expression/moment, what to hide, brand elements. Prepared before filming so you know what shot/expression to capture.

### SCRIPT / VIDEO PLAN (Module 9)
Write the hook and CTA fully; bullet the setup; leave the body open per the format template matching this video type.

### FILM (Modules 4/5/10/12)
Run the Pre-Filming Checklist Card. Film with verbal timestamp markers for standout moments. Do pickup lines/inserts immediately after the main session while fresh.

### SEND TO EDITOR (Module 13)
Complete and send the Editor Brief template with organized, clearly labeled footage and flagged highlight timestamps.

### REVIEW
One consolidated, timestamped feedback pass distinguishing objective fixes from subjective preference (Module 13, Section 6).

### UPLOAD (Module 1/7/8)
Publish with the selected title and thumbnail, timed to trend relevance where applicable (reaction content especially).

### SHORTS (Module 14)
Extract 2–4 Shorts from retention-spike moments (once initial data is in) or pre-identified strong beats. Draft hooks/captions for each.

### TIKTOK/REELS (Module 15)
Adapt the same clips with platform-native adjustments (captions, trending audio where it fits naturally).

### PROMOTION (Module 15/16)
Community post teaser, light-touch social posts, Discord/X engagement around the launch.

### ANALYTICS (Module 17)
Log the video in your Per-Video Analytics Log once meaningful data accumulates (typically 48–72 hours post-upload for an initial read, with a further check at 7 days).
...
```
[Day] Idea review + scoring + selection: ___
[Day] Title + thumbnail concept drafting: ___
[Day] Scripting: ___
[Day] Filming: ___
[Day] Editor brief sent: ___
[Day] Review + feedback pass: ___
[Day] Upload + promotion push: ___
[Day] Shorts/cross-platform posting (spread across week): ___
