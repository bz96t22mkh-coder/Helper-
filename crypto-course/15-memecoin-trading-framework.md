...
One primary, testable meme-coin trading framework — not ten strategies — that combines market condition (Module 9), discovery (Module 10), narrative/X vetting (Module 11), tokenomics (Module 12), security (Module 13), on-chain/chart reliability (Module 14), and risk management, into a single repeatable, 11-stage process — a market-condition check, discovery, qualification, research, entry criteria (with a confirmation requirement built in), stop/invalidation, position sizing, target/exit, trade management, no-trade conditions, and risk controls.
