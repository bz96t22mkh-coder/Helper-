...
**Session and liquidity behavior on BTC**, despite the market technically never closing, still shows real, observable clustering — it isn't uniformly active around the clock. Historically, volume and volatility concentrate more heavily around U.S. and Asian trading hours, when the trading desks, institutions, and retail populations of those regions are most active, and around scheduled macro data releases like CPI, FOMC decisions, and NFP, which move BTC because of its correlation to broader risk sentiment (§3). This is conceptually similar in spirit to the killzone framework you already use in futures trading — recurring windows where liquidity and directional moves are more likely — but the underlying driver is different: futures killzones are anchored to a fixed session structure and daily close, while BTC's active windows are anchored to macro calendar events and regional liquidity patterns layered on top of a market that never actually shuts. Module 6 will work through precisely how to adapt your killzone-based timing instincts to this different underlying mechanism.
