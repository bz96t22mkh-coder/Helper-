...
**Trading and buy/sell behavior** — the ratio and pattern of buy versus sell transactions over time, visible on the block explorer — tells you something aggregate volume numbers alone can't: *who* is driving the activity. The specific thing to isolate is whether large sell transactions are coming from top or insider wallets specifically, versus being spread broadly across many different holders. A token where sells are broadly distributed across a large number of small holders looks like normal, healthy profit-taking and rotation in a liquid market. The same aggregate sell volume concentrated in a small number of large, early wallets looks like exactly what it usually is — insiders exiting into buying pressure that retail traders are providing, often without realizing who's on the other side of their trade. Distinguishing these two patterns is the direct, practical payoff of having done the wallet-level work in Section 4; without knowing which wallets are early/insider wallets, you can't tell these two very different situations apart from the trade tape alone.

**Age of the token** functions as a dial, not a pass/fail filter, and it's worth being precise about why. A newer token carries categorically higher risk simply because there's been less time for anything about it to prove itself out — less time for the contract's behavior to be observed under real trading pressure, less time for a liquidity lock to be tested, less time for a genuine (or fake) community to reveal its true character. But that same newness is also where the largest percentage moves in this entire asset class happen, precisely because early is when the most attention and capital can still flow in relative to the current market cap. The correct use of age, then, is as one input that shifts how much scrutiny everything else needs and how much of your capital a position deserves — a three-hour-old token demands a stricter pass on every other check in this module, and a smaller position size even if it passes, not an automatic rejection.

**Community, narrative, and social growth** is where this module closes the loop back to Module 11 — apply that module's tools directly here rather than treating fundamentals and social analysis as separate tracks. Look for the same signature of genuine versus manufactured growth (diversifying, original engagement from many independent accounts versus repetitive, bot-like, or tightly clustered activity — Module 11 §6), and separately ask whether the token's narrative currently fits a sector theme that's actively strong (Module 9's sector rotation) or is trying to force a connection to a theme that's already cooling. A token can pass every fundamental check in this module — clean holder distribution, locked liquidity, renounced contract — and still be a poor trade candidate if its narrative has no genuine attention behind it; fundamentals tell you whether a token is *safe enough to consider*, and this final synthesis step tells you whether there's any real reason to expect attention and capital to actually arrive.
