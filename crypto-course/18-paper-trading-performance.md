...
1. **Sample size:** at least 30 logged paper trades (or a combination of backtested and paper trades) in that specialization. Thirty is not an arbitrary round number — it's roughly the point at which basic statistics stop being dominated by noise and start giving you a signal you can trust even a little. A five-trade winning streak tells you almost nothing about your actual edge; it's entirely consistent with a mediocre or even negative-expectancy framework that simply got lucky over a short stretch. Below this sample size, any performance number — good or bad — should be treated as provisional, not as evidence of readiness either way.
