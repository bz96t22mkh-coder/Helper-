...
**Fake tokens and copycat contracts** are one of the most common real-world meme-coin scam vectors, and unlike the other threats in this section it targets the discovery moment itself, before you've even reached a site or an X account: a scammer deploys a brand-new contract using the identical name and ticker symbol as a trending, legitimate token, sometimes within minutes of the real one gaining attention, betting that traders searching by name on a screener or a chat will click the wrong contract address and buy the impostor instead. Because DEX Screener, Birdeye, and similar tools let anyone search by name or ticker, and because a name and ticker cost nothing to copy, this scam requires no technical sophistication at all — only speed and a trending name to imitate. **Direct check:** never trust a name or ticker alone; verify the actual contract address against a primary source — the real project's own official account or website — before buying, every time, even (especially) when a token is moving fast and the name looks exactly right.
