...
- **ETH's liquidity, deep but structurally different from BTC's,** shows up in where the volume sits: spot and derivatives volume on major CEXs is comparable in depth to BTC's for normal position sizes — ETH is still a "major," not a thin altcoin — but a meaningfully larger share of ETH's liquidity sits on-chain, spread across DEX pools on Ethereum mainnet and its L2s (Arbitrum, Base, Optimism) rather than concentrated on a handful of centralized order books. That split matters practically: a large ETH order routed through a single DEX pool can see more slippage than the same dollar size would on a deep CEX order book, even though ETH's *total* liquidity across all venues is enormous — so where you execute, not just how much you're trading, affects your realized price. This is a difference of degree from BTC, not of kind, and it's the same liquidity-fragmentation logic you'll apply far more sharply to L2 tokens and meme coins later (Module 10 §3).
